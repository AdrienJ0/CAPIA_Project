...
if __name__ == "__main__":
    ...
    model_infos_path = "Models/model_with_mask_plein_clahe20240508"
    ...
    model_infos_text_path = os.path.join(files_path, "model_infos_training.txt")
    ...
    masks_pred_dir_path = model_infos_path + "/masks_predicted_training"
    results_path = model_infos_path + "/results_training"
    create_dir(masks_pred_dir_path)
    create_dir(results_path)

    """ Loading model """
    with CustomObjectScope({'iou': iou, 'dice_coef': dice_coef, 'dice_loss': dice_loss}):
        model = tf.keras.models.load_model(files_path + "/model.h5", compile=False)

    """ Load the dataset """
    test_x = sorted(glob(os.path.join(split_dataset_path, "train_mask_plein_clahe", "images", "*")))
    test_y = sorted(glob(os.path.join(split_dataset_path, "train_mask_plein_clahe", "masks", "*")))
...
if __name__ == "__main__":
  ...
  df.to_csv(files_path + "/score_training.csv")